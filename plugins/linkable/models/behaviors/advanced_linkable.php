...
class AdvancedLinkableBehavior extends ModelBehavior {
			...
			do {
				...
				foreach ($iterations as $alias => $options) {
					...
					if (isset($associations[$Reference->alias])) {						
						...
						$refAssociations = $Reference->getAssociated();
						$refAssociationType = $refAssociations[$_Model->alias];
						$_Model->bindModel(array('belongsTo' => array($Reference->alias => $Reference->{$refAssociationType}[$_Model->alias])));
						$type = 'belongsTo';
						$association = $_Model->{$type}[$Reference->alias];
					...
					if (empty($options['conditions'])) {
						if ($type === 'belongsTo') {						
							...
							$options['conditions'] = array("{$modelKey} = {$referenceKey}");
							...
							$options['conditions'] = array("{$modelLink} = {$modelKey}");
							...
							$options['conditions'] = array("{$referenceKey} = {$modelKey}");
						}
					}
					
					if (is_string($association['conditions'])) {
						$association['conditions'] = array($association['conditions']);
					}
					// merge with conditions defined in the associations if foreignKey is set in the association settings
					if (isset($association['foreignKey']) && $association['foreignKey'] !== false) {
